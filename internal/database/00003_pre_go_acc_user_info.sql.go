...
// versions:
//   sqlc v1.28.0
// source: 00003_pre_go_acc_user_info.sql
...
SELECT user_id, user_account, user_nickname, user_avatar, user_state, user_mobile, user_gender, user_birthday, user_email, user_is_authentication, created_at, updated_at FROM ` + "`" + `pre_go_acc_user_info` + "`" + ` WHERE user_account LIKE ? OR user_nickname LIKE ?
`
...
func (q *Queries) FindUsers(ctx context.Context, arg FindUsersParams) ([]PreGoAccUserInfo, error) {
	rows, err := q.db.QueryContext(ctx, findUsers, arg.UserAccount, arg.UserNickname)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []PreGoAccUserInfo
	for rows.Next() {
		var i PreGoAccUserInfo
		if err := rows.Scan(
			&i.UserID,
			&i.UserAccount,
			&i.UserNickname,
...
func (q *Queries) GetUser(ctx context.Context, userID uint64) (PreGoAccUserInfo, error) {
...
func (q *Queries) GetUsers(ctx context.Context, userID uint64) ([]PreGoAccUserInfo, error) {
...
SELECT user_id, user_account, user_nickname, user_avatar, user_state, user_mobile, user_gender, user_birthday, user_email, user_is_authentication, created_at, updated_at FROM ` + "`" + `pre_go_acc_user_info` + "`" + ` LIMIT ? OFFSET ?
`
...
func (q *Queries) ListUsers(ctx context.Context, arg ListUsersParams) ([]PreGoAccUserInfo, error) {
