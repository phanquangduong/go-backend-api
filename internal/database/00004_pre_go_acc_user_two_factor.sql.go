...
// versions:
//   sqlc v1.28.0
// source: 00004_pre_go_acc_user_two_factor.sql
...
func (q *Queries) GetTwoFactorMethodByID(ctx context.Context, twoFactorID uint32) (PreGoAccUserTwoFactor, error) {
	row := q.db.QueryRowContext(ctx, getTwoFactorMethodByID, twoFactorID)
	var i PreGoAccUserTwoFactor
	err := row.Scan(
		&i.TwoFactorID,
		&i.UserID,
		&i.TwoFactorAuthType,
...
func (q *Queries) GetTwoFactorMethodByIDAndType(ctx context.Context, arg GetTwoFactorMethodByIDAndTypeParams) (PreGoAccUserTwoFactor, error) {
...
func (q *Queries) GetUserTwoFactorMethods(ctx context.Context, userID uint32) ([]PreGoAccUserTwoFactor, error) {
	rows, err := q.db.QueryContext(ctx, getUserTwoFactorMethods, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []PreGoAccUserTwoFactor
	for rows.Next() {
		var i PreGoAccUserTwoFactor
		if err := rows.Scan(
			&i.TwoFactorID,
