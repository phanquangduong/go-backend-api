...
-- name: FindUsers :many
SELECT * FROM `pre_go_acc_user_info` WHERE user_account LIKE ? OR user_nickname LIKE ?;

-- name: ListUsers :many
SELECT * FROM `pre_go_acc_user_info` LIMIT ? OFFSET ?;


-- name: RemoveUser :exec
DELETE FROM `pre_go_acc_user_info` WHERE user_id = ?;


-- -- name: UpdatePassword :exec
-- UPDATE `pre_go_acc_user_info` SET user_password = ? WHERE user_id = ?;



-- name: AddUserAutoUserId :execresult
INSERT INTO `pre_go_acc_user_info` (
